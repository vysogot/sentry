...
class ProjectPreprodCheckForUpdatesEndpointTest(APITestCase):
    ...
    def _get_url(self):
        return reverse(
            "sentry-api-0-project-preprod-artifact-check-for-updates",
    ...
    def test_current_artifact_with_build_configuration(self):
        """Test current artifact retrieval with build_configuration filter"""
        self._create_android_artifact(
            main_binary_identifier="test-identifier",
            build_configuration_id="debug",
        )
    ...
    def test_platform_specific_filtering_android(self):
        ...
        assert data["update"] is not None
        assert (
            data["update"]["build_version"] == "1.0.0"
    ...
    def test_multiple_artifacts_same_version_different_build_configurations(self):
        ...
        self._create_android_artifact(
            ...
            build_version="1.0.0",
            build_number=50,
            build_configuration_id="release",
        )
        ...
        assert data["current"] is not None
        assert data["current"]["build_configuration_id"] == "debug"
